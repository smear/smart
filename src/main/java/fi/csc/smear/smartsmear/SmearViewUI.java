...
public class SmearViewUI extends UI {
	...
	protected void init(VaadinRequest request) {
		...
		final Station stations = new Station(variableselection); 
		...
		for (int i = 0; i < Station.ASEMAT.length; i++){
			...
			for (int j = 0; j < Station.TAULUT[i].length; j++){
				...
				try{
					...
					System.out.println("Exception in retrieving tablemetadata");
				}
        ...
        startdate.addListener(changeValueListener_from);
        enddate.addListener(changeValueListener_to);
        startdate.setImmediate(true);
        enddate.setImmediate(true);
				...
				if (!set2.isEmpty()){					
					...
					if (null != hdf5) {
						...
						new Notification("Could not produce HDF5",
								"<br/>Could not produce HDF5", Notification.TYPE_WARNING_MESSAGE, true)
						.show(Page.getCurrent());
					} 
				}
		...
		timewindow.addListener(new Field.ValueChangeListener() {
        	public void valueChange(ValueChangeEvent event) {
        		if (timewindow.getValue() == "Day"){
		...
		typeavaraging.addListener(new Field.ValueChangeListener() {
        	public void valueChange(ValueChangeEvent event) {
		...
		avaraging.addListener(new Field.ValueChangeListener() {
        	public void valueChange(ValueChangeEvent event) {
